...
func NewUserRepository(db *mysql.Dialect) *UserRepo {
...
func (r *UserRepo) GetUserByID(id int) (*modules.User, error) {
	...
	err := r.db.DB.Get(&user, "SELECT * FROM users WHERE id = ? AND deleted_at IS NULL", id)
	if err != nil {
...
func (r *UserRepo) CreateUser(user *modules.User) (int64, error) {
	result, err := r.db.DB.Exec("INSERT INTO users (name, email, password, age) VALUES (?, ?, ?, ?)",
...
func (r *UserRepo) CreateUserWithAudit(user *modules.User) (int64, error) {
	...
	res, err := tx.Exec("INSERT INTO users (name, email, password,  age) VALUES (?, ?, ?, ?)", user.Name, user.Email, user.Password, user.Age)
	if err != nil {
		tx.Rollback()
...
func (r *UserRepo) GetUserByEmail(email string) (*modules.User, error) {
	var user modules.User
	err := r.db.DB.Get(&user, "SELECT * FROM users WHERE email=? AND deleted_at IS NULL", email)
	if err != nil {
		return nil, err
	}
	return &user, err
